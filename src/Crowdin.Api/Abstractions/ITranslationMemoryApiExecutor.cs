...
namespace Crowdin.Api.Abstractions
{
    ...
    {
        Task<ResponseList<TranslationMemory.TranslationMemory>> ListTms(
            int? userId = null,
            int? groupId = null,
            int limit = 25,
            int offset = 0,
        ...
        Task<ResponseList<TranslationMemory.TranslationMemory>> ListTms(TmsListParams @params);

        Task<TranslationMemory.TranslationMemory> AddTm(AddTmRequest request);

        Task<TranslationMemory.TranslationMemory> GetTm(int tmId);

        Task DeleteTm(int tmId);

        Task<TranslationMemory.TranslationMemory> EditTm(int tmId, IEnumerable<TmPatch> patches);
